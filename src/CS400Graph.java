...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            //clone the copyPath dataSequence (deep copy)
    ...
    protected Path dijkstrasShortestPath(T start, T end) {  
        //if the vertices dont exist
        if(!vertices.containsKey(start) || !vertices.containsKey(end)){
            throw new NoSuchElementException("Key does not exist");
        }
        LinkedList<Vertex> visited = new LinkedList<>();
        PriorityQueue<Path> pq = new PriorityQueue<>();
        Vertex startingV = vertices.get(start);
        //add starting vertex to visited list
        visited.add(startingV);
        //adding nodes from starting node to successors in priority queue
        for(int i = 0; i< startingV.edgesLeaving.size(); ++i){
            Path p = new Path(new Path(startingV), startingV.edgesLeaving.get(i));
            pq.add(p);
        }

        //while priority queue is not empty then remove the shortest path 
        while(!pq.isEmpty()){
            Path p = pq.remove();
            //get the vertex were visiting and add it to visited list
            Vertex visitingV = p.end;
            visited.add(visitingV);
            //if the vertex were visiting is the end vertex of djikstra's then return it
            if(visitingV.equals(vertices.get(end))){
                return p;
            }
            //iterate through the list of edges leaving this visiting vertex
            for (int i = 0; i < visitingV.edgesLeaving.size(); i++) {
                //if the successor vertex has not been visited then extend it with the current path 
                //and add that path to pq
                if(!visited.contains(visitingV.edgesLeaving.get(i).target)){
                    pq.add(new Path(p, visitingV.edgesLeaving.get(i))); 
                }
            }
        }
        //no such shortest path found, no such element exception
